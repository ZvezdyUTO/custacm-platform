...
- `multiple`：从受保护用户目录取得允许采集且已绑定 OJ 的用户，以最大并发数 6 拉取个人汇总并生成多人视图；“显示退役队员”会用 `includeRetired=true` 重读目录；结果按总过题量降序排列，同分时按 `username` 升序排列，查询失败项置于末尾；筛选参数变更后自动刷新，无需查询按钮。
- `single`：按 `username`、OJ、日期和难度范围查询个人汇总、提交与首 AC；队员下拉以 `username · nickname` 展示并降序排列，“选择退役队员”会读取包含已退役账号的目录。
- `problem`：按题目、OJ 和日期范围查询提交与首 AC。
- `admin/create-users`：文本导入后生成可编辑信息行，也可逐行新增或删除，确认后批量创建账号并绑定 OJ handle。
- `admin/users`：在已加载账号中按 username 子串即时查询，并承载编辑、改名、密码、角色、OJ handle、采集状态和删除用户；更换已有 OJ handle 时使用高危确认弹窗，确认后调用独立接口清理该 OJ 的全部历史训练数据、旧绑定与采集状态再换绑。永久删除前弹窗说明 OJ 绑定、训练数据清理和评论匿名化影响。空头像回退到构建内置默认头像，固定 `root` 不显示删除入口。
- `admin/articles`：筛选文章、控制首页侧栏精选状态，并在明确提示文章、标签关联和评论不可恢复后执行管理员永久删除。
- `admin/categories`：分类可自定义名称和颜色；标签只允许新增和删除，新增颜色由服务端从连续数值空间随机生成并持久化。有关联文章时后端拒绝删除，Blog 标签云从全部标签中随机展示最多 30 个，并使用深色背景和白字。
- `admin/training`：按 OJ 对全部或单个现役队员发起采集并查看任务；首次成功采集前抓取全部历史，之后从该用户、该 OJ 上次成功窗口结束时间向前倒退指定小时数并采集至当前时间。执行前弹窗确认目标、OJ、倒退范围和数仓刷新影响，每次采集都固定在完成后刷新数仓。
...
该接口默认只返回可采集且至少绑定一个 OJ 账号的用户摘要；传入 `includeRetired=true` 时保留已退役用户：
...
| `src/api/training.ts` | 支持可选 `includeRetired` 的用户目录和多人/单人/题目训练查询 |
| `src/api/admin.ts` | 用户、文章精选、文章分类、OJ handle、采集任务、数仓刷新与首页图片管理 |
| `src/composables/useAuthSession.ts` | 会话恢复、登录竞态隔离、退出与密码修改 |
| `src/composables/usePlatformDashboard.ts` | 查询、分页、用户管理、采集与刷新状态编排；采集任务结束后重读管理员用户目录以同步 OJ 采集进度 |
...
| `src/components/AppShell.vue` | 训练内容外壳；独立开发时提供与生产一致的调试顶栏，管理员页面不激活“训练中心”；同源嵌入 Blog 时不渲染第二条顶栏 |
| `src/components/LoginPanel.vue` | 登录表单和安全回跳 |
| `src/components/TrainingQueryPanel.vue` | 多人、单人筛选参数短防抖后自动刷新并可包含退役队员；更新时间旁说明空日期、Rating 边界的实际含义；单人查询不预选队员，以 username 优先的降序下拉供选择；题目查询保留深色显式查询按钮 |
...
| `src/components/AdminUserManagementPanel.vue` | 按 username 子串过滤已加载账号，以只读头像和宽幅个人信息列集中展示账号、邮箱与 OJ handle，并管理已有用户的角色、密码和采集状态；保存成功后自动收起编辑区，保存失败时保留现场；已有 handle 变化必须经过高危清理确认；头像仅能通过本人图片上传接口更新 |
| `src/components/TrainingDataOpsPanel.vue` | 全部/单人采集、固定数仓刷新与采集任务记录，并按当前 OJ 展示增量采集游标及最近成功窗口结束时间 |
