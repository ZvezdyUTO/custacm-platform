  ...
  <section class="training-data-collection-panel admin-reference-page" aria-label="训练数据采集">
    <header class="reference-page-header collection-reference-header">
      ...
      <div class="collection-reference-controls">
        ...
        <button class="primary-button collect-all-button" :disabled="isOjBusy(collectionOj) || !collectableUsers.length" type="button" @click="collectAll"><RefreshCw :class="{ spin: isOjBusy(collectionOj) }" :size="18" />{{ isOjBusy(collectionOj) ? '正在采集' : '全部采集' }}</button>
      </div>
    </header>
    ...
    <div v-if="pendingCollection" class="collection-confirm-backdrop" role="presentation" @click.self="pendingCollection = null">
      <section class="collection-confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="collection-confirm-title" aria-describedby="collection-confirm-description">
        ...
        <div class="collection-confirm-actions"><button type="button" @click="pendingCollection = null">取消</button><button class="confirm-collection-button" type="button" @click="confirmCollection"><RefreshCw :size="16" />确认执行采集</button></div>
      </section>
    </div>
  </section>
</template>
...
interface PendingCollection { usernames: string[]; targetLabel: string; lookbackHours: number; ojName: OjName }
const collectionOj = ref<OjName>(OJ_NAMES.CODEFORCES); const globalLookback = ref<string | number>('1440'); const lookbackByUsername = ref<Record<string, string | number>>({}); const collectErrors = ref<Partial<Record<OjName, string>>>({}); const locallyRunningOjs = ref(new Set<OjName>()); const expandedJobs = ref(new Set<string>()); const pendingCollection = ref<PendingCollection | null>(null);
...
const collectError = computed(() => collectErrors.value[collectionOj.value] ?? '');
const runningOjs = computed(() => {
  const result = new Set(locallyRunningOjs.value);
  jobs.value.forEach((job) => {
    if ((job.status === 'PENDING' || job.status === 'RUNNING') && job.ojName) result.add(job.ojName);
  });
  return result;
});
...
function setCollectError(ojName: OjName, message: string) { collectErrors.value = { ...collectErrors.value, [ojName]: message }; }
function isOjBusy(ojName: OjName) { return runningOjs.value.has(ojName); }
function collectAll() { const usernames = collectableUsers.value.map((item) => item.user.username); if (!usernames.length) return; let lookback; try { lookback = parseLookback(globalLookback.value); } catch (error) { setCollectError(collectionOj.value, error instanceof Error ? error.message : '倒退小时数无效。'); return; } pendingCollection.value = { usernames, targetLabel: `全部 ${usernames.length} 名队员`, lookbackHours: lookback, ojName: collectionOj.value }; }
function collectOne(username: string) { let lookback; try { lookback = parseLookback(lookbackByUsername.value[username] || globalLookback.value); } catch (error) { setCollectError(collectionOj.value, error instanceof Error ? error.message : '倒退小时数无效。'); return; } const user = collectableUsers.value.find((item) => item.user.username === username); pendingCollection.value = { usernames: [username], targetLabel: user?.user.nickname ? `${username} · ${user.user.nickname}` : username, lookbackHours: lookback, ojName: collectionOj.value }; }
function confirmCollection() {
  const request = pendingCollection.value;
  if (!request) return;
  if (isOjBusy(request.ojName)) {
    setCollectError(request.ojName, `${OJ_LABELS[request.ojName]} 采集任务正在运行。`);
    pendingCollection.value = null;
    return;
  }
  setCollectError(request.ojName, '');
  locallyRunningOjs.value = new Set([...locallyRunningOjs.value, request.ojName]);
  pendingCollection.value = null;
  void runCollection(request.usernames, request.lookbackHours, request.ojName)
    .catch((error) => { setCollectError(request.ojName, error instanceof Error ? error.message : '采集失败。'); })
    .finally(() => {
      const remaining = new Set(locallyRunningOjs.value);
      remaining.delete(request.ojName);
      locallyRunningOjs.value = remaining;
    });
}
