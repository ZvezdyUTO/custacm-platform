  ...
  <section class="training-data-collection-panel admin-reference-page" aria-label="训练数据采集">
    ...
    <div class="collection-member-list">
      <article v-for="item in collectableUsers" :key="item.user.username" class="collection-member-row">
        <div class="collection-member-identity"><strong>{{ item.user.username }}{{ item.user.nickname || '' }}</strong><span>{{ OJ_LABELS[collectionOj] }}：{{ item.handles[collectionOj] }}</span></div>
...
const collectionOj = ref<OjName>(OJ_NAMES.CODEFORCES); const globalLookback = ref<string | number>('1440'); const lookbackByUsername = ref<Record<string, string | number>>({}); const collectError = ref(''); const allBusy = ref(false); const busyUsers = ref(new Set<string>()); const expandedJobs = ref(new Set<string>());
...
async function runCollection(usernames: string[], lookbackHours: number) { await props.dashboard.batchCollectSubmissions(collectionRequest(usernames, lookbackHours, collectionOj.value)); }
async function collectAll() { const usernames = collectableUsers.value.map((item) => item.user.username); if (!usernames.length) return; let lookback; try { lookback = parseLookback(globalLookback.value); } catch (error) { collectError.value = error instanceof Error ? error.message : '回看小时数无效。'; return; } if (!window.confirm(`确认采集全部 ${usernames.length} 名队员的 ${OJ_LABELS[collectionOj.value]} 提交并刷新数仓？`)) return; allBusy.value = true; collectError.value = ''; try { await runCollection(usernames, lookback); } catch (error) { collectError.value = error instanceof Error ? error.message : '采集失败。'; } finally { allBusy.value = false; } }
async function collectOne(username: string) { let lookback; try { lookback = parseLookback(lookbackByUsername.value[username] || ''); } catch (error) { collectError.value = error instanceof Error ? error.message : '回看小时数无效。'; return; } const next = new Set(busyUsers.value); next.add(username); busyUsers.value = next; collectError.value = ''; try { await runCollection([username], lookback); } catch (error) { collectError.value = error instanceof Error ? error.message : '采集失败。'; } finally { const done = new Set(busyUsers.value); done.delete(username); busyUsers.value = done; } }
