...
`GET /profiles/{username}` 位于公开 GET 层，只返回文章作者主动公开展示的头像、nickname、username、email、签名、友情链接和本人已开启 `profileVisible` 的 `achievements`，不暴露 role、密码或 OJ handle，也不提供任何修改能力。`GET /player/me`、本人资料/友情链接修改和头像更新响应返回全部本人获奖记录及各自 `profileVisible`，供个人页管理展示偏好。

`GET /competitions` 与 `GET /competitions/{id}` 位于公开 GET 层，只返回未删除的比赛聚合；年份、类型标签和分页参数只改变查询结果，不改变匿名访问权限。回收站比赛及其个人获奖记录对 Guest、Player 和普通公开查询一律隐藏。
...
- Vue Blog 的个人页为 `GET /player/me`、`PATCH /player/me/profile`、`PUT /player/me/profile-links`、`POST /player/me/avatar` 和奖项展示偏好 PUT 显式附加 Bearer token；本人资料响应附带全部 `achievements` 及 `profileVisible`，头像文件必须先在浏览器裁剪为 512×512 PNG。
