...
- 账号变更以数据库返回的规范 username 执行身份保护与 handle 操作；即使请求使用 `ROOT` 等能被数据库匹配的写法，也不能绕过 `root` 保护。
...
- 管理员用户更新使用单一原子入口；先锁定账号行，再读取当前绑定并决定清理范围，移除或更换 handle 前必须清理对应 OJ 数据。
- 修改、删除账号在访问数据库前取得两个 OJ 的采集租约，并持有至事务提交或回滚；采集或刷新进行中返回 409，避免清理后仍有旧 handle 的采集写入。
