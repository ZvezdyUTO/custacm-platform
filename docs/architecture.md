...
Vue Blog 外壳通过公开 `GET /site` 获取最小初始化数据：奖励图、评论管理员标记、旧动态直达页需要的头像/名称，以及分类、标签和精选文章。Blog API 对四项站点配置使用专用查询和独立缓存，不再执行未展示的最新文章查询，也不组装旧徽章、收藏、社交链接或滚动文字。
...
唯一 Blog API 运行时默认启用两种 OJ 的提交采集。每个用户、每个 OJ 独立保存最近一次成功采集窗口的结束时间；没有成功记录时从数据源历史起点采集，有记录时从该时间向前倒退配置时长并采集至本次任务开始时间。Codeforces 每天 00:00、AtCoder 每天 00:15 倒退 120 小时；其余时段分别在每小时的 `00/30` 分与 `15/45` 分倒退 1 小时。所有规则使用 `Asia/Shanghai`，AtCoder 错开 15 分钟以避免共享调度线程和外部 API 请求同时启动。
...
`GET /player/training-data/users` 只向已登录用户返回成员的 `username`、`nickname`、`ojNames`，不暴露真实 OJ handle 或管理员字段；默认排除已退役成员，只有查询参数 `includeRetired=true` 才保留。Vue 3 多人查询在浏览器侧以最大并发数 6 消费该目录。
