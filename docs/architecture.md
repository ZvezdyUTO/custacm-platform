...
- 仓库刷新覆盖本批次及受影响的旧、新首 AC 日期；重复 AC 不扩大到无关历史。每个 OJ 在当前 JVM 内保留一个未完成日期区间，节点部分失败后的重试从完整 DAG 重建，后续新批次合并该区间；启用仓库刷新的采集即使没有新提交也可恢复。仅完整成功后清除；该恢复状态不跨进程重启持久化。
...
- 文章归档在短只读事务中读取一致的元数据快照，释放数据库连接后再编码 ZIP 和向客户端输出。全站元数据仍一次驻留内存，图片内容按流复制。
