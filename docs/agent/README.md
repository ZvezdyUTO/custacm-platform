...
- `platform-training-data/training-data-web` is the second runnable backend implementation. It exposes admin-only Codeforces ODS batch-upsert, recent-lookback collection, process-local collection-job start/list/detail, warehouse refresh, handle-account, and student-data purge APIs, and applies Codeforces ODS/DWD/DWM/DWS plus handle-account table migrations.
- `platform-common` currently contains empty shared Maven modules.
- `frontend` is the first runnable frontend slice. It is a React/Vite/TypeScript admin workbench that logs in through `auth-web` and reads real auth/training-data APIs through local dev and Nginx same-origin proxies. The training query workspace has single-user detail query and automatic-collection multi-user summary pages sharing date/rating filters, and uses the public auth user list plus per-user Codeforces handle lookup so guest users can choose real players. The admin workspace separates user information management, training-data synchronization/collection, and operation records after admin login; user information includes an all-user list sorted by descending student-number prefix with existing-user edits expanded inside the list; data synchronization starts Codeforces collection as backend jobs, shows per-user collection status, polls an expandable collection-task list, and gates high-cost collection/deletion operations behind confirmation. Frontend workspace tabs are mirrored to URL paths under `/query/*` and `/admin/*` so refresh and direct links preserve the current page.
