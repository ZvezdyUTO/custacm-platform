...
| `training-data-common` | `GET` | `/api/training-data/codeforces/first-accepted/by-student` | 否 | 游客按 `studentIdentity` 和可选 `ojName` 查询指定 OJ 首 AC 题目明细 |
| `training-data-common` | `GET` | `/api/training-data/codeforces/first-accepted/by-problem` | 否 | 游客按 `problemKey` 和可选 `ojName` 查询指定 OJ 首 AC handle 列表 |
...
`training-data-web` 启动时也会应用内部 Codeforces ODS/DWD/DWM/DWS 表迁移、AtCoder ODS 落地表迁移、common 模块里的 AtCoder DWD/DWM/DWS 建表迁移和 `oj_handle_account` 表迁移；AtCoder ODS 迁移、Kenkoooo source client、ODS parser/writer、submission collector、problem-list collector 和 DWD/DWM/DWS SQL task refresh 由 `training-data-atcoder` 提供，AtCoder DWD/DWM/DWS 表迁移由 `training-data-common` 提供。Codeforces 采集按单个 `user.status?handle=...` 页面执行，团队提交归因给本次采集的目标 handle，并以 `submission + handle` 粒度写入 ODS/DWD。DWD/DWM/DWS 转换以各 OJ 模块内的 SQL 任务资源表示，并由采集任务内部 refresh handler 触发；当前刷新任务覆盖 Codeforces 和 AtCoder。
