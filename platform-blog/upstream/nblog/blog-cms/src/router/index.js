...
const routes = [
	...
	},
	{
		...
		children: [
			...
			},
			{
				...
				meta: {title: '写动态', icon: 'el-icon-edit', roles: ['ROLE_admin']}
			},
			{
				path: 'edit/:id',
				name: 'EditBlog',
			...
			},
			{
				...
				name: 'EditMoment',
				component: () => import('@/views/blog/moment/WriteMoment'),
				meta: {title: '编辑动态', icon: 'el-icon-edit', roles: ['ROLE_admin']},
				hidden: true
			},
			{
				path: 'list',
			...
			},
			{
				...
				meta: {title: '动态管理', icon: 'el-icon-chat-dot-round', roles: ['ROLE_admin']}
			},
			{
				path: 'category/list',
				name: 'CategoryList',
				component: () => import('@/views/blog/category/CategoryList'),
				meta: {title: '分类管理', icon: 'el-icon-s-opportunity', roles: ['ROLE_admin']}
			},
			{
				path: 'tag/list',
				name: 'TagList',
				component: () => import('@/views/blog/tag/TagList'),
				meta: {title: '标签管理', icon: 'biaoqian', roles: ['ROLE_admin']}
			},
			{
				path: 'comment/list',
				name: 'CommentList',
				component: () => import('@/views/blog/comment/CommentList'),
				meta: {title: '评论管理', icon: 'el-icon-s-comment', roles: ['ROLE_admin']}
			},
		]
	},
	{
		...
		redirect: '/page/site',
		component: Layout,
		meta: {title: '页面管理', icon: 'el-icon-document-copy', roles: ['ROLE_admin']},
		children: [
			{
				path: 'site',
				name: 'SiteSetting',
	...
	},
	{
		...
		component: Layout,
		meta: {title: '图床管理', icon: 'el-icon-picture', roles: ['ROLE_admin']},
		children: [
			{
				path: 'setting',
				name: 'Setting',
	...
	},
	{
		...
		children: [
			...
			},
			{
				...
				meta: {title: '定时任务', icon: 'el-icon-alarm-clock', roles: ['ROLE_admin']}
			},
		]
	},
	{
		...
		redirect: '/log/job',
		component: Layout,
		meta: {title: '日志管理', icon: 'el-icon-document', roles: ['ROLE_admin']},
		children: [
			{
				path: 'job',
				name: 'JobLog',
	...
	},
	{
		...
		component: Layout,
		meta: {title: '数据统计', icon: 'el-icon-s-data', roles: ['ROLE_admin']},
		children: [
			{
				path: 'visitor',
				name: 'Visitor',
...
router.beforeEach((to, from, next) => {
	if (to.path !== '/login') {
		...
		const forbidden = to.matched.some(route => route.meta.roles && !route.meta.roles.includes(user.role))
		if (forbidden) return next(user.role === 'ROLE_player' ? '/blog/list' : '/404')
	}
