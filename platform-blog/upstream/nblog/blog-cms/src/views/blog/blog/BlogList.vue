<template>
	<div>
		...
		<el-table :data="blogList">
			...
			<el-table-column label="置顶" width="80" v-if="isAdmin">
				<template v-slot="scope">
					<el-switch v-model="scope.row.top" @change="blogTopChanged(scope.row)"></el-switch>
				</template>
			</el-table-column>
			<el-table-column label="推荐" width="80" v-if="isAdmin">
				<template v-slot="scope">
			...
			<el-table-column label="可见性" width="100">
				<template v-slot="scope">
					<el-link :icon="isAdmin ? 'el-icon-edit' : ''" :underline="false" @click="isAdmin && editBlogVisibility(scope.row)">
...
<script>
	...
	export default {
		...
		computed: {
			isAdmin() {
				return JSON.parse(window.localStorage.getItem('user') || '{}').role === 'ROLE_admin'
			}
		},
		created() {
			this.getData()
