...
describe('Blog account menu', () => {
	it('keeps profile and logout for a player', () => {
		expect(accountMenuItems({role: 'ROLE_player'}).map(item => item.label)).toEqual(['个人资料', '退出登录'])
	})

	it('adds the admin entry only for an administrator', () => {
		expect(accountMenuItems({role: 'ROLE_admin'}).map(item => item.label)).toEqual(['个人资料', '管理员界面', '退出登录'])
	})
})
