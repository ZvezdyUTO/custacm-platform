...
- 顶栏搜索只按标题子串匹配已发布文章，并按最近更新时间返回最多十条候选；游客只获得公开文章，登录用户的显式 Bearer 读取还会获得内部文章，草稿始终不进入聚合接口。
- 普通用户只能管理本人文章；管理员可管理全部文章，新建文章的作者固定为当前认证管理员。
- 普通用户与管理员文章写接口统一限制标题 100 字、简介 255 字、正文 200000 字，不能依赖前端输入限制保证数据合法。
- Markdown 输出在服务端按 HTML 白名单清洗，移除脚本、事件属性和危险 URL 协议，避免文章内容形成存储型 XSS；围栏代码块仅保留 Prism 所需的 `language-*` class 供前端高亮，数学公式范围内的反斜杠在 CommonMark 转换前后保持原样，保证阅读页与编辑器使用相同的 LaTeX 换行语义。
...
- 持久化本人 nickname、公开展示的 email、个性签名，以及最多八条按用户隔离和排序的 HTTP(S) 友情链接。
...
- 默认启用 Codeforces 与 AtCoder 提交自动采集：未成功采集过的用户先抓取全部历史；之后两种 OJ 每天各从上次成功窗口结束时间向前倒退 120 小时，其余时段每 30 分钟倒退 1 小时并采集至当前时间；AtCoder 相对 Codeforces 错开 15 分钟，避免同时访问外部数据源。
...
| `src/main/resources/application.properties` | Blog API 基础设施配置，以及默认开启的 Codeforces/AtCoder 每日恢复与半小时增量采集规则 |
...
| `controller/PublicProfileController.java`、`model/vo/PublicProfile.java` | 匿名只读的文章作者公开名片接口，返回 username、nickname、email、头像、签名和友情链接，不暴露角色或 OJ handle |
...
| `model/vo/BlogDetail.java`、`model/vo/BlogInfo.java`、`model/vo/SearchBlog.java`、`mapper/BlogMapper.xml` | 公开/内部文章详情、按认证态筛选的列表与标题搜索；搜索候选返回文章简介，游客聚合排除内部文章，登录用户聚合包含内部文章；文章 DTO 和查询不包含阅读数 |
...
| `controller/IndexController.java`、`service/impl/SiteSettingServiceImpl.java` | 组装最小公开 `/site` 响应；只查询并返回页面仍消费的奖励图、评论管理员标记、旧动态页头像/名称、分类、标签和精选文章，不再查询最新文章或组装旧徽章/收藏配置 |
...
| `model/vo/AdminUserMutationResponse.java`、`service/impl/AdminUserService.java` | 管理员用户响应同时返回 OJ handle、采集资格，以及每个 OJ 最近一次成功采集所覆盖窗口的结束时间 |
| `controller/player/TrainingDataQueryController.java` | 认证后的训练查询与支持可选 `includeRetired` 的用户目录；注入无 MVC 注解的 `OjWarehouseQueryFacade`，不代理训练模块 Controller。 |
| `db/migration/` | Blog baseline、训练 schema、身份与外键整合、首页图片、用户资料、托管图片资产、文章可见性，以及移除文章阅读数字段和同步任务的迁移 |
...
| `src/test/java/top/naccl/controller/IndexControllerTest.java`、`src/test/java/top/naccl/service/impl/SiteSettingServiceImplTest.java` | 锁定 `/site` 最小响应字段及公开配置查询范围，防止旧徽章、收藏和最新文章查询回流 |

## 训练用户目录

`GET /player/training-data/users` 要求 `ROLE_player` 或 `ROLE_admin`，默认只列出 `needCollect=true` 且至少绑定一个 OJ 账号的用户；传入 `includeRetired=true` 时不再按 `needCollect` 排除已退役用户。每项字段严格为：
