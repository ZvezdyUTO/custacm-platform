...
- 提供文章 ZIP 下载、管理员全量文章备份、分类、标签、公开/内部评论、公开比赛与获奖记录、公开作者资料和首页横幅 API。
...
公开接口为 `GET /competitions` 和 `GET /competitions/{id}`。列表接受可选 `startYear`、`endYear`、`type` 以及 `pageNum`/`pageSize`；年份上下界组成包含端点的闭区间，只传一端时按单侧范围过滤，`type` 匹配比赛拥有的任一类型标签。比赛根以全称作为正常记录唯一标识，并记录 `INDIVIDUAL`、`TEAM` 或 `MIXED` 参赛形态。

一场比赛可同时拥有多个类型标签。请求使用的稳定代码及含义为：
...
创建比赛时，`LANQIAO_CUP` 不能与 `PROVINCIAL` 标签同时出现；`BAIDU_STAR` 允许使用 `PROVINCIAL` 标签表达省赛。这一类型组合校验与下文奖项 `awardScope` 校验同时生效。
...
除回收站恢复外不提供比赛、参赛用户或奖项的 PUT/PATCH；需要更正时删除旧子记录后重新添加。已被奖项引用的参赛用户要先删除相关奖项，再删除参赛记录。奖项形态只允许 `INDIVIDUAL` 或 `TEAM`，等级必须为 1–4，排名拆成正整数 `rankPosition`/`rankTotal` 并满足前者不大于后者，响应字段 `rank` 的值形如 `(x/y)`。`INDIVIDUAL` 必须且只能绑定一名参赛用户且不能填队名；`TEAM` 至少绑定两名参赛用户，可保存可选队名。普通比赛的 `awardScope` 可省略或使用 `PROVINCIAL`/`NATIONAL`；蓝桥杯必须为 `NATIONAL`，百度之星与团体程序设计天梯赛必须从省级和国家级范围中选择一项。
...
新获奖关系默认 `profileVisible=false`。获奖人使用 `PUT /player/competitions/{competitionId}/awards/{awardId}/profile-visibility` 和 `{"visible":true|false}` 修改本人名片展示偏好；团队成员独立设置，管理员也不能代替其他人修改。该开关只影响资料名片，不影响比赛详情中的获奖人列表。

`GET /player/me`、`PATCH /player/me/profile`、`PUT /player/me/profile-links` 和 `POST /player/me/avatar` 的本人资料响应包含全部 `achievements` 及 `profileVisible`；公开 `GET /profiles/{username}` 只返回开启展示的项目。每项获奖记录带比赛全称/年份/类型、奖项形态与范围、1–4 级、名称和 `(x/y)` 排名。参赛关系以 `username` 外键关联账号并在改名时级联；删除账号时 username 置空，但昵称快照、参赛记录和历史奖项继续保留。
...
| `src/main/java/top/naccl/controller/player/PlayerCompetitionController.java` | 参赛用户本人公开文章的比赛绑定/解绑和名片奖项展示偏好 |
| `src/main/java/top/naccl/service/CompetitionService.java`、`mapper/CompetitionMapper.java` | 比赛聚合校验、批量装配、文章可见性、个人获奖记录与七天清理 |
...
| `src/main/java/top/naccl/enums/Competition*.java`、`model/dto/Competition*.java`、`model/vo/Competition*.java` | 比赛类型/形态/范围约束及请求、公开树和个人获奖投影 |
...
| `src/main/resources/db/migration/V037__create_competition_records.sql` | 创建比赛根、类型、参赛用户、奖项/获奖人、个人展示偏好和文章关联，并约束正常名称唯一与 username 生命周期 |
| `src/test/java/top/naccl/UnifiedSchemaMigrationTest.java` | 锁定统一迁移及 V034/V035/V036/V037 schema 合同 |
