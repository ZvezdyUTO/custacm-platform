...
- `training-data-codeforces`：Codeforces 入口、ingest app service、最近小时数 submission 采集器、进程内采集任务服务、学生数据清理用例、批次上下文、ODS record/parser/writer、CF handle-account 维护、ODS/DWD/DWM/DWS DDL、幂等 SQL 任务、SQL 任务 manifest、admin warehouse refresh 入口、fixture/tests。
- `training-data-web`：Spring Boot 入口、模块健康检查、模块信息、文件日志、Flyway 迁移、OJ-specific HTTP 写入入口、admin 最近小时数采集入口、admin 采集任务查询入口和 admin SQL refresh 运行时。
...
任务按多个 `studentIdentity` 逐个采集并可在每个成功批次后刷新仓库。状态只保存在当前 `training-data-web` 进程内，适合浏览器刷新或切换页面后继续查看，不是持久化 pipeline run state，后端重启后不会恢复。
...
管理员可以创建绑定、迁移绑定的 `studentIdentity`，并切换 `need_collect` 是否参与后台自动采集；这些接口不修改 handle，也不修改 auth 登录账号。游客可以按 `studentIdentity` 查询单个 Codeforces handle 绑定。

管理员可以删除某个 `studentIdentity` 当前 Codeforces 绑定及其训练数据：

```text
DELETE /api/training-data/admin/codeforces/users/{studentIdentity}/data
```

该接口先解析当前绑定的 Codeforces handle，再删除该 handle 在 ODS、DWD、DWM、DWS 中的行，最后删除 `codeforces_handle_account` 绑定。没有绑定时返回成功且计数为 0。它不删除 auth 账号；彻底删除用户需要调用方继续调用 auth-web 的用户删除接口。

Codeforces 内部 DWD/DWM/DWS app 查询以平台 `studentIdentity` 作为个人维度入口；app service 会先解析到绑定的 Codeforces handle，再复用按 `author_handle` 建模的仓储和数仓表查询。DWS 也提供面向自动采集用户的列表查询：读取 `need_collect=true` 的绑定，逐个复用个人区间 rating AC 汇总，并按总 AC 数降序返回。提交明细 DWD 查询由后端按 `page + limit` 分页，按提交时间和 submission id 倒序返回，并在每次响应中返回精确 `total`、`totalPages` 和 `hasMore`。问题维度查询结果也会反查 `studentIdentity`，遇到未绑定 handle 会失败。

这些 DWD/DWM/DWS 读侧能力已经公开为无需鉴权的 guest HTTP 查询接口，路径位于 `/api/training-data/codeforces/accepted-summary`、`/api/training-data/codeforces/accepted-summary/auto-collect-users`、`/api/training-data/codeforces/submissions/by-*` 和 `/api/training-data/codeforces/first-accepted/by-*`。
