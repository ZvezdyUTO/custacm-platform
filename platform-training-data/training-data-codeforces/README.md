...
It owns Codeforces ODS ingest, Codeforces handle-account mapping, ODS/DWD/DWM/DWS schema migrations, idempotent SQL task resources, internal Java read-side query services, local fixtures, and tests. Personal app-layer read queries use platform `studentIdentity` and resolve it to a Codeforces handle internally before querying warehouse repositories. It does not expose public DWD/DWM/DWS query HTTP APIs yet.
...
| --- | --- | --- | --- | --- | --- |
| DWD personal report | `CodeforcesSubmissionQueryService.listStudentSubmissions` | `CodeforcesHandleSubmissionCriteria` | `CodeforcesSubmissionRepository.findHandleSubmissions` | `dwd_codeforces__submission` | App input is `studentIdentity` plus optional UTC+8 submitted time range and problem rating bounds; app resolves the handle, repository filters by handle, and app returns identity/handle submission detail items. |
| DWD problem report | `CodeforcesSubmissionQueryService.listProblemSubmissions` | `CodeforcesProblemSubmissionCriteria` | `CodeforcesSubmissionRepository.findProblemSubmissions` | `dwd_codeforces__submission` | Requested problem key and optional UTC+8 submitted time range; app returns matching submission detail items with `studentIdentity + authorHandle`, failing if any result handle is unbound. |
| DWM personal report | `CodeforcesFirstAcceptedProblemQueryService.summarizeStudentFirstAcceptedProblems` | `CodeforcesHandleFirstAcceptedProblemCriteria` | `CodeforcesFirstAcceptedProblemRepository.findHandleFirstAcceptedProblems` | `dwm_codeforces__handle_problem_first_accepted` | App input is `studentIdentity` plus optional UTC+8 first-accepted time range and problem rating bounds; app resolves the handle, repository filters by handle, and app returns identity/handle first-accepted problem details. |
