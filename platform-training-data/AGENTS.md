...
- Implemented Codeforces handle-account storage target is `codeforces_handle_account`, keyed by `student_identity` with a unique immutable Codeforces handle and `need_collect` automatic-collection flag.
- Codeforces owns its own HTTP ingress, ingest application service, `studentIdentity`-scoped recent-lookback submission collector with bounded source request timeout/retry, in-process collection job service for frontend polling, student-data purge use case, collect batch type, ODS record, parser, writer, handle-account mapping, fixture, DDL, ODS upsert SQL, DWD/DWM/DWS SQL task resources, SQL task manifest, Spring config, and tests.
- `training-data-web` exposes module health/info, OJ-specific ODS batch-upsert endpoints under `/api/training-data/admin/**`, Codeforces admin recent-lookback collection by `studentIdentity`, Codeforces admin collection-job start/list/detail endpoints, Codeforces admin handle-account write endpoints, Codeforces admin student-data purge, Codeforces admin warehouse refresh, public Codeforces handle lookup by `studentIdentity`, and public Codeforces DWD/DWM/DWS query endpoints including the automatic-collection user DWS summary list.
- OJ-specific ODS batch-upsert endpoints require the platform `admin` role and must stay under the admin URL tier.
- Codeforces handle-account creation and identity/collection-flag updates require the platform `admin` role. Public Codeforces handle lookup by `studentIdentity` is a guest endpoint and must not parse JWTs.
- Codeforces student-data purge requires the platform `admin` role, deletes only training-data-owned Codeforces binding and ODS/DWD/DWM/DWS rows for the resolved handle, and must not delete auth accounts directly.
...
- Keep the current slice focused on Codeforces warehouse modeling, Codeforces handle-account mapping, OJ-specific batch upsert, recent-lookback submission collection, student-data cleanup, SQL task resources, and the current admin-triggered SQL task refresh path.
...
- Codeforces handle-account identity migration changes the `student_identity` key in `codeforces_handle_account` and may update `need_collect`; it does not modify auth accounts and must not change the stored Codeforces handle.
